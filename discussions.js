...
window.RISE_DISCUSSIONS = async function({client,user,profile,root,report}) {
 const teacher=['teacher','admin'].includes(profile.role);
 ...
 async function open(topic,answerPage=0){
  ...
  try{
   ...
   const answers=checked(await client.from('rise_discussion_answers').select('*').eq('topic_id',topic.id).order('created_at').order('id').range(answerPage*20,answerPage*20+20));
   ...
   if(!teacher&&!topic.closed&&!answers.length){
 ...
 async function draw(){
  ...
  try{
   ...
   for(const topic of rows.slice(0,12)){
    const card=document.createElement('article');card.className='auth-record';card.innerHTML=`<h3>${esc(topic.title)}</h3><p>${topic.closed?'已結束回答':'開放回答中'} · ${esc(new Date(topic.created_at).toLocaleDateString('zh-TW'))}</p>`;
    const button=document.createElement('button');button.textContent=teacher?'查看題目與學生回答':'閱讀題目與回答';button.onclick=()=>open(topic);card.append(button);$('#discussion-list').append(card);
   }
