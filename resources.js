(() => {
  ...
  function organizer(){
    ...
    box.innerHTML=`<h2>整理教材版面</h2><p>將所選教材歸入同一主題／單元，按上下按鈕調整順序。此操作不會改變草稿或發布狀態。</p><form id="organize-form"><div class="resource-fields"><label>發布區域<select id="organize-destination"><option value="science">科學探索</option></select></label><label>主題顯示
    名稱<input id="organize-collection" required maxlength="100" value="${esc(selected[0].collection||'')}"></label><label>主題顯示順序<input id="organize-order" type="number" min="0" max="999999" step="1" value="${selected[0].group_order||0}"></label></div><p class="auth-help">每位教師、每個學科的單元會分開顯示。要調整整個單元，請選取其中全部教材。</p><ol id="organize-list"></ol><button type="submit">套用歸類與順序</button> <button type="button" id="organize-close" class="secondary">關閉</button><p id="organize-status" role="status"></p></form>`;
  ...
  function editor(r=null){
    ...
    box.innerHTML=`<h2>${r?'編輯資源':'新增資源'}</h2><form id="res-form">
      ...
      <div class="auth-field"><label for="res-collection">主題顯示名稱</label><input id="res-collection" required maxlength="100" placeholder="例如：牛頓運動定律" value="${esc(r?.collection||'')}"><p class="auth-help">同一學科、同一名稱的教材會收在同一張單元卡片內。</p></div></div>
      <div class="resource-fields"><div class="auth-field"><label for="res-group-order">主題顯示排序</label><input id="res-group-order" type="number" min="0" max="999999" step="1" value="${r?.group_order||0}"></div><div class="auth-field"><label for="res-item-order">主題內教材順序</label><input id="res-item-order" type="number" min="0" max="999999" step="1" value="${r?.item_order||0}"></div></div><p class="auth-help">數字越小越前。也可儲存後選取多份教材，使用上下按鈕整理順序。</p>
  ...
  async function start(){
    try{
      ...
      if(!canEdit()&&!new URLSearchParams(location.search).has('id')){
        root.innerHTML='<section class="auth-card"><h2>教材管理僅限教師與管理員</h2><p>學生與助教可從科學探索閱讀已發布教材。</p><a class="auth-button" href="science.html">前往科學探索</a></section>';
        return;
      }
      root.innerHTML=`${canEdit()?'<section class="auth-card"><h2>資源管理</h2><p>教師可管理自己建立的資源；管理員可管理全部內容。</p><div class="auth-actions"><button id="res-new">新增資源</button><button id="res-mode" class="secondary">管理草稿與已發布內容</button><button id="res-organize" class="secondary">整理所選教材（0）</button></div></section>':''}<div class="resource-destinations" aria-label="教學資源區域"><button type="button" id="res-science">科學探索區</button></div><section id="resource-organizer" class="auth-card" hidden></section><section id="resource-editor" class="auth-card" hidden></section><section id="resource-detail" class="auth-card" hidden></section><h2 id="resource-list-title">已發布資源</h2><div id="resource-list" class="resource-grid"></div><div class="auth-actions"><button id="res-prev">上一頁</button><button id="res-next">下一頁</button></div>`;
      ...
      function setDestination(value){if(busy)return;destination=value;offset=0;$('#res-science').setAttribute('aria-pressed',String(value==='science'));list().catch(e=>tell(errorMessage(e),true));}
      $('#res-science').onclick=()=>setDestination('science');
      $('#res-science').setAttribute('aria-pressed',String(destination==='science'));
