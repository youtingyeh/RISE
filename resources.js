(() => {
  ...
  async function start(){
    try{
      ...
      root.innerHTML=`${canEdit()?'<section class="auth-card"><h2>資源管理</h2><p>教師可管理自己建立的資源；管理員可管理全部內容。</p><div class="auth-actions"><button id="res-new">新增資源</button><button id="res-mode" class="secondary">管理草稿與已發布內容</button><button id="res-organize" class="secondary">整理所選教材（0）</button></div></section>':''}<div class="resource-destinations" aria-label="教學資源區域"><button type="button" id="res-science">科學探索區</button><button type="button" id="res-learning">學習路徑區</button></div><section id="resource-organizer" class="auth-card" hidden></section><section id="resource-editor" class="auth-card" hidden></section><section id="resource-detail" class="auth-card" hidden></section><h2 id="resource-list-title">已發布資源</h2><div id="resource-list" class="resource-grid"></div><div class="auth-actions"><button id="res-prev">上一頁</button><button id="res-next">下一頁</button></div>`;
      ...
      $('#res-science').onclick=()=>setDestination('science');$('#res-learning').onclick=()=>setDestination('learning');
