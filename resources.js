(() => {
  ...
  async function list(){
    ...
    for(const r of rows.slice(0,12)){
      ...
      card.innerHTML=`<p class="eyebrow">${esc(labels[r.subject])} · ${esc(labels[r.kind])} · ${esc(labels[r.status])}</p><h2>${esc(r.title)}</h2><p>${esc(r.summary)}</p><p class="auth-help">更新：${esc(new Date(r.updated_at).toLocaleDateString('zh-TW'))}</p>`;
  ...
  function editor(r=null){
    ...
    const box=$('#resource-editor');box.hidden=false;
    ...
    $('#res-form').onsubmit=async event=>{
      ...
      try{
        ...
        const doc={title:$('#res-title').value.trim(),summary:$('#res-summary').value.trim(),body:$('#res-body').value,subject:$('#res-subject').value,kind:$('#res-kind').value,status:requestedStatus,youtube_id,files};
  ...
  async function start(){
    try{
      ...
      root.innerHTML=`${canEdit()?'<section class="auth-card"><h2>資源管理</h2><p>教師可管理自己建立的資源；管理員可管理全部內容。</p><div class="auth-actions"><button id="res-new">新增資源</button><button id="res-mode" class="secondary">管理草稿與已發布內容</button></div></section>':''}<section id="resource-editor" class="auth-card" hidden></section><section id="resource-detail" class="auth-card" hidden></section><h2 id="resource-list-title">已發布資源</h2><div id="resource-list" class="resource-grid"></div><div class="auth-actions"><button id="res-prev">上一頁</button><button id="res-next">下一頁</button></div>`;
      if(canEdit()){$('#res-new').onclick=()=>editor();$('#res-mode').onclick=()=>{mode=mode==='published'?'manage':'published';offset=0;$('#resource-list-title').textContent=mode==='published'?'已發布資源':profile.role==='admin'?'全部資源（含草稿）':'我的資源（含草稿）';$('#res-mode').textContent=mode==='published'?'管理草稿與已發布內容':'返回公開列表';list().catch(e=>tell(errorMessage(e),true));};}
