...
begin
 ...
 if p_area='assignments' then
  return jsonb_build_object('items',(select coalesce(jsonb_agg(to_jsonb(a) order by a.created_at desc),'[]') from rise_work.assignments a where a.published or (r in ('teacher','admin') and (a.owner_id=u or r='admin'))));
 elsif p_area='assignment' then
  if not exists(select 1 from rise_work.assignments where id=p_id and (published or (r in ('teacher','admin') and (owner_id=u or r='admin')))) then raise exception 'rise:無權查看作業。' using errcode='42501';end if;
